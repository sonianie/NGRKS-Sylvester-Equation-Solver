...
% Call the residual computation function for the NGRKS method
[iter,RES1,RES2] = res_NGRKSn_sylv(A,B,F,G,1,As1,As2,Bs1,Bs2,tol);

% Plot the computed residuals and their theoretical bounds
figure(1)
hold on
box on
plot((1:length(RES1)),log10(RES1),'r-', 'LineWidth', 2.5);
plot((1:length(RES2)),log10(RES2),'b*-', 'LineWidth', 2.5);
xlabel('Iter');
ylabel('log_{10}(Res)');
legend('real valus of Res', 'upper bounds given by (4.7)');
