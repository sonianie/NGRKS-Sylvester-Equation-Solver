function [iter,R1,R2] = res_NGRKSn_sylv(A,B,F,G,km,As1,As2,Bs1,Bs2,tol)
% Residual norm Estimation of Novel Global Rational Krylov Subspace method for Sylvester matrix equations: AX+XB+F*G'=0
% Input:
...
%       R1:          the true residual norm of NGRKS(km)
%       R2:          the estimated residual norm of NGRKS(km)
...
while iter < 200
    ...
    % Calculate the residual norm using the low-dimensional solution Y
       ...
       res2  = sqrt(norm(ttMA*Y,'fro')^2+norm(Y*ttMB','fro')^2);  
    % Calculate the true residual norm using the full-form solution Xm 
       Xm   = V(:,1:iter*p+p)*kron(Y,speye(p))*W(:,1:iter*p+p)';  
       res1 = norm(A*Xm+Xm*B+F*G','fro');
       R1  = [R1;res1];R2  = [R2;res2];
       
    % Check for convergence
       if res2 < tol*nrmFG;break;end
