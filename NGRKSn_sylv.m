function [Z1,Z2,iter,RES] = NGRKSn_sylv(A,B,F,G,As1,As2,Bs1,Bs2,km,tol)
% Novel Global Rational Krylov Subspace method for Sylvester matrix equations: AX+XB+F*G'=0
% Input:
...
%       tol:         tolerance
% Output
%       Z1,Z2:       approximate solution X=Z1*Z2';
%       iter:        the number of iterations of NGRKS(km)
%       RES:         the residual norm of NGRKS(km)
...
% Update the projected matrix of A(B) on V(W)
    ...
    % Calculate the residual norm using the low-dimensional solution Y
       ...
       RES  = [RES;res];
    % Print the current iteration and residual
       fprintf('NGRKS-Iter %d, res = %e\n',iter,res);
